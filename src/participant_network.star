...
zond_metrics_exporter = import_module(
    "./zond_metrics_exporter/zond_metrics_exporter_launcher.star"
)
...
def launch_participant_network(
    ...
    for index, participant in enumerate(args_with_right_defaults.participants):
        ...
        if participant.zond_metrics_exporter_enabled:
            pair_name = "{0}-{1}-{2}".format(index_str, cl_type, el_type)

            zond_metrics_exporter_service_name = (
                "zond-metrics-exporter-{0}".format(pair_name)
            )

            zond_metrics_exporter_context = zond_metrics_exporter.launch(
                plan,
                pair_name,
                zond_metrics_exporter_service_name,
                el_context,
            ...
            plan.print(
                "Successfully added {0} zond metrics exporter participants".format(
                    zond_metrics_exporter_context
                )
            )
    ...
    for index, participant in enumerate(args_with_right_defaults.participants):
        ...
        if participant.zond_metrics_exporter_enabled:
            zond_metrics_exporter_context = all_zond_metrics_exporter_contexts[
                index
            ]
        xatu_sentry_context = None
